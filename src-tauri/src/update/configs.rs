...
use crate::{resolve::structs::{LauncherPath, ResolveData}, get_version};
...
pub async fn resolve_configs(app: &tauri::AppHandle, path: &PathBuf, launcher: String, _custom: bool) {
    ...
    if ver.version != ver.latest_version {
        download_file(&client, &path.join("conf.zip"), "https://drive.google.com/uc?export=download&id=1qa7gThngkqNooUweuyVs6Kes8w_pIJ0l&confirm=t").await.unwrap();
...
async fn resolve_lconfigs(path: &PathBuf, ltype: String, app: &tauri::AppHandle) {
    ...
    if ltype == "curseforge" {
        ...
        download_file(&client, &path.join("curseforge.zip"),"https://drive.google.com/uc?export=download&id=1BKqLWvB287lv6zgGhGp3UOyr6Dy0jeIe&confirm=t").await.unwrap();
        update_progress(90, app);
        ...
        download_file(&client, &path.join("prism.zip"), "https://drive.google.com/uc?export=download&id=18r_C-tvMEjcbBUA8TqApOXMkqaR_XFrs&confirm=t").await.unwrap();
        update_progress(90, app);
